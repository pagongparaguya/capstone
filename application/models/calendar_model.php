...
$this->conf['template'] = '
   ...
   {heading_next_cell}<th><div class ="rightarrow pull-right"><a href="{next_url}">&gt;&gt;</a></div></th>{/heading_next_cell}
...
$query=$this->db->select('date')->from('closed_dates')->get();
$ctr=0;
$closed=array();
...
public function get_calendar_data($year,$month){
	/*$query=$this->db->select('date,status')->where from('closed_dates')->like('date',"$year-$month",'after')->get();
		*/
		$query=$this->db->select('date')->from('closed_dates')->like('date',"$year-$month",'after')->get();
		$ctr=0;
...
public function close_sched($date){
	
	if(!$this->db->select('date')->from('closed_dates')->where('date',$date)->count_all_results()){
		$data = array('date'=>$date);
	$this->db->insert('closed_dates',$data);
		}

}

public function open_sched($date){
	$this->db->where('date',$date);
$this->db->delete('closed_dates');
}
